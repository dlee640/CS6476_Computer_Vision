...
def DFT_matrix(N):
    ...
    for m in range(N):
        for n in range(N):
            U[m, n, 0] = (1/N) * torch.cos(torch.tensor(2 * torch.pi * m * n) / N)
            U[m, n, 1] = (-1/N) * torch.sin(torch.tensor(2 * torch.pi * m * n) / N)
...
def my_dft(img):
    ...
    temp = Compl_mul_Real(U, img)
    dft = Compl_mul_Compl(temp, U)
...
def dft_filter(img):
    ...
    dft = my_dft(img)
    N = img.size()[0]
    mask = torch.zeros(N, N, 2)
    mask[0, 0, :] = 1
    mask[0, N-1, :] = 1
    mask[N-1, 0, :] = 1
    mask[N-1, N-1, :] = 1
    temp = dft * mask
    ifft_result = torch.ifft(temp, 2)
    img_back = ifft_result[:, :, 0]
