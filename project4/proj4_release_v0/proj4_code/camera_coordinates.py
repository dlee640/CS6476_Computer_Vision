...
def transformation_matrix(wRc_T, wtc):
    ...
    #############################################################################
    # TODO: YOUR CODE HERE
    r_bottom = np.array([0., 0., 0., 1])
    temp = np.hstack((wRc_T, np.matmul(-wRc_T, wtc.T)))
    M = np.vstack((temp, r_bottom))
...
def convert_3d_points_to_camera_coordinate(M, points_3d_w):
    ...
    # TODO: YOUR CODE HERE
    # Extract size
    n = points_3d_w.shape[0]
    m = points_3d_w.shape[1]

    # Create dummy output
    points_3d_c = np.zeros((n, 4))

    # Append column of ones if input isn't homogeneous
    if m is 3:
        points_3d_w = np.hstack((points_3d_w, np.ones((n,1))))

    # Row by row matrix multiplication between M & input
    for i in range(n):
        points_3d_c[i, :] = np.matmul(M, points_3d_w[i, :].T)
...
def projection_from_camera_coordinates(K, points_3d_c):
    ...
    # TODO: YOUR CODE HERE
    # Extract size
    n = points_3d_c.shape[0]
    m = points_3d_c.shape[1]

    # Create dummy output & required matrix [I|0]
    pp_2d_homogeneous = np.zeros((n, 3))
    projected_points_2d = np.zeros((n,2))
    IO = np.hstack((np.eye(3), np.zeros((3,1))))

    # Append column of ones if input isn't homogeneous
    if m is 3:
        points_3d_c = np.hstack((points_3d_c, np.ones((n,1))))

    # Row by row matrix multiplication between M & input
    for i in range(n):
        pp_2d_homogeneous[i, :] = np.matmul(K, np.matmul(IO, points_3d_c[i, :].T))
        projected_points_2d[i, :] = pp_2d_homogeneous[i, 0:2]/pp_2d_homogeneous[i,2]
