...
def objective_func(x, **kwargs):
    ...
    projected_points_2d = projection(P, points_3d)
    diff = (points_2d - projected_points_2d).flatten()
    ############################################################################
...
def projection(P: np.ndarray, points_3d: np.ndarray) -> np.ndarray:
    ...
    for i in range(n):
        projected_points_2d[i,0] = ((P[0,0] * points_3d[i,0] + P[0,1] * points_3d[i,1] 
                                  + P[0,2] * points_3d[i,2] + P[0,3]) /
         (P[2,0] * points_3d[i,0] + P[2,1] * points_3d[i,1] + P[2,2] * points_3d[i,2] + P[2,3]))

        projected_points_2d[i,1] = ((P[1,0] * points_3d[i,0] + P[1,1] * points_3d[i,1] 
                                  + P[1,2] * points_3d[i,2] + P[1,3]) /
         (P[2,0] * points_3d[i,0] + P[2,1] * points_3d[i,1] + P[2,2] * points_3d[i,2] + P[2,3]))
...
def estimate_camera_matrix(pts2d: np.ndarray, 
    ...
    P = least_squares(objective_func, initial_guess, method='lm', verbose=0, 
                      max_nfev=50000, kwargs={'pts2d':pts2d, 'pts3d':pts3d},
                      ftol=1e-8, gtol=1e-8, xtol=1e-8)

    P = np.append(P.x, 1.).reshape(3,4)
    ############################################################################
...
def calculate_camera_center(P: np.ndarray,
    ...
    #############################################################################
    # TODO: YOUR CODE HERE
    result = np.matmul(np.linalg.inv(np.matmul(K, R_T)), P)
    cc = -result[:, -1].T
