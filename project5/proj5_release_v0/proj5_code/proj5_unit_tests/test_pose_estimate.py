...
def test_pose_estimate(test_img='data/player1.jpg'):
  ...
  expected_left_thumb = torch.Tensor([185.7245, 363.2718])
  detected_left_thumb = torch.Tensor(land_mark[22])
  assert torch.allclose(expected_left_thumb, detected_left_thumb)
...
def test_projection_2d_to_3d():
  ...
  assert torch.allclose(torch.Tensor(pose2d), torch.Tensor(pose2d_reconstruct))
